...
namespace FarmHand
{
    ...
    public class ModEntry : Mod
    {
        ...
        private static Vector2 clickedTile;
        /*********
        ** Public methods
        *********/
        ...
        private void Input_ButtonPressed(object sender, ButtonPressedEventArgs e)
        {
            ...
            this.Monitor.Log($"{player.Name} @ {player.getTileLocation()} pressed {e.Button} @ {e.Cursor.Tile}", LogLevel.Debug);

            clickedTile = e.Cursor.Tile;                                //{X:3 Y:10}
        }
        ...
        private void OnUpdateTicked(object sender, UpdateTickedEventArgs e)
        {
            ...
            Vector2 playerCurrentTile = player.getTileLocation();
            this.Monitor.Log($"{clickedTile}");

            if (playerCurrentTile.Y < clickedTile.Y)
            {
                player.SetMovingUp(false);
                player.SetMovingDown(true);
                player.setMoving((byte)48);
                player.yVelocity = 2;
                this.Monitor.Log("Set moving down", LogLevel.Debug);
            }
            if (playerCurrentTile.Y > clickedTile.Y)
            {
                ...
                player.setMoving((byte)48);
                player.yVelocity = 2;
                this.Monitor.Log("Set moving up", LogLevel.Debug);
            }
            if (playerCurrentTile.X > clickedTile.X)
            {
                player.SetMovingLeft(true);
                player.SetMovingRight(false);
                player.setMoving((byte)48);
                player.xVelocity = 2;
                this.Monitor.Log("Set moving left", LogLevel.Debug);
            }
            if (playerCurrentTile.X < clickedTile.X)
            {
                ...
                player.setMoving((byte)48);
                player.xVelocity = 2;
                this.Monitor.Log("Set moving right", LogLevel.Debug);
            }
            this.Monitor.Log(player.getMovementSpeed().ToString(), LogLevel.Debug);
            this.Monitor.Log(player.xVelocity.ToString(), LogLevel.Debug);
            this.Monitor.Log(player.yVelocity.ToString(), LogLevel.Debug);
        }
        ...
        public static void StartPatching()
        {
            ...
            MethodInfo farmer_Halt_PrefixPatch = AccessTools.Method(typeof(ModEntry), "PrefixMethod_Farmer_HaltPatch");
            ...
            MethodInfo farmer_MovePosition_PrefixPatch = AccessTools.Method(typeof(ModEntry), "PrefixMethod_Farmer_MovePositionPatch");
            ...
            MethodInfo game1_UpdateControlInput_PostfixPatch = AccessTools.Method(typeof(ModEntry), "PostfixMethod_Game1_UpdateControlInputPatch");
